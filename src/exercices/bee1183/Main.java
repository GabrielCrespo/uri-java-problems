...
public class Main {
    ...
    public static final int QUANTITY_OF_ELEMENTS_ABOVE_MAIN_DIAGONAL = (MATRIX_COLUMN_SIZE * (MATRIX_COLUMN_SIZE - 1)) / 2;
    ...
    public static float sumAboveMainDiagonal(float[][] array) {
        ...
        for (int i = 0; i < MATRIX_LINE_SIZE; i++) {
            for (int j = i+1; j < MATRIX_COLUMN_SIZE; j++) {
                sum += array[i][j];
            }
        }
    ...
    public static float average(float[][] array) {
        ...
        return sum / QUANTITY_OF_ELEMENTS_ABOVE_MAIN_DIAGONAL;
    }
